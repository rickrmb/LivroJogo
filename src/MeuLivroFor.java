		...
		//if(i>0 ^ i<0) {//nao consigo implementar regra de perda de vidas
			//for (i = 3; i > 1; i--) {
				...
				while (reiniciar.equalsIgnoreCase("SIM")) {
					...
					System.out.println("\nMuito bem " + nome
							+ "! Você está pronto para recuperar o agilidium? Responda SIM ou NÃO.");
					decisao = leitor.next();

					while (!decisao.equalsIgnoreCase("SIM") & !decisao.equalsIgnoreCase("Não") & i!=0) {
						System.out.println("\nEu lá sei o que é " + "'" + decisao + "'"
								+ "! Você perdeu uma vida, e agora só tem mais " + i
					...
					if (decisao.equalsIgnoreCase("SIM")) {
						...
						while (!caminho.equalsIgnoreCase("cascata") & !caminho.equalsIgnoreCase("sprintolandia") & i!=0) {
							System.out.println(
									"\nVocê tem um sério problema com digitação né? Você perdeu uma vida, e agora só tem mais " + i + " vida! Vamos tentar de novo.\nCASCATA ou SPRINTOLANDIA?");
							caminho = leitor.next();
							i--;
						}
